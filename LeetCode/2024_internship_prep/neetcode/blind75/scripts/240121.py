...
def findWords(board, words):
    ...
    for i, row in enumerate(board):
        for j, val in enumerate(row):
            ...
            T[val].append(prev_start)

    def _recursive(i, j, word, k, prev):
        if k == len(word):
            return True
        ...
        for mi, mj in MOVES:
            ...
            if ii < 0 or ii == R or jj < 0 or jj == C:
                continue
            if board[ii][jj] != word[k] or (ii, jj) in prev['visited']:
                continue
            if word[:k+1] in T:
                skip = False
                for p in T[word[:k+1]]:
            ...
            curr = deepcopy(prev)
            curr['last'] = (ii, jj)
            curr['visited'].add((ii, jj))
            print(curr['visited'], ' - ', curr['last'])
            T[word[:k+1]].append(curr)

            if _recursive(ii, jj, word, k+1, curr):
                return True
            
        return False
    
    def _search_by_ref(word, k):
        if k == 0:
            return False

        print(word[:k])
        if word[:k] in T:
            for prev in T[word[:k]]:
                i, j = prev['last']
                if _recursive(i, j, word, k, prev):
                    return True
        
        return _search_by_ref(word, k-1)
    
    for word in words:
        if word[0] not in T:
            continue
        if _search_by_ref(word, len(word)):
            result.append(word)
...
if __name__ == '__main__':
    ...
    print('result : {}'.format(findWords(board, words)))